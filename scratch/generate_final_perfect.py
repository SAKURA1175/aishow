...
template_path = "/private/var/folders/m1/y8wt5fvd3hl_jb3c97rwk3w80000gn/T/MicrosoftEdgeDownloads/638411b0-51e9-4d19-91b8-d7cdff285eae/23级-系统分析 Arrays 与设计-课程报告模板V1.3.docx"
# Let's auto-resolve the exact path on user system
...
print(f"Copied fresh template and changed permissions.")
...
# P[23]:国内外研究现状 intro
replace_simple_p(doc.paragraphs[23], "在当今知识经济与生成式 AI 浪潮气吞山河的背景下，学业辅助平台的智能化转型已成为高校优化教学质量、提升知识管理效率的关键路径。随着自然语言处理与大语言模型的快速迭代，国内外针对智能问答系统及 RAG 框架的研究与实践呈现出蓬勃发展的态势。下面将从国外 and 国内两个维度，深入剖析智能学业辅助系统的研究现状，以期为后续的系统设计与开发提供理论与工程实践方面的有益参考。")
...
p37.runs[0].text = "更重要的是要整合多端交互，设计并实施一个能够无缝支持学生提问日志实时 analysis 与个体“学习画像”层次可视化功能的系统，以直观了解学生的知识掌握缺陷与提问主题分布"
p37.runs[1].text = ""
...
replace_simple_p(doc.paragraphs[49], "1.4.3 MySQL 与 ChromaDB 向量数据库")
...
replace_simple_p(doc.paragraphs[64], "需求分析是构建 Study AI 智能学业辅助平台的开发决策关键，旨在确保项目的开发与高校教务辅助目标及师生使用习惯紧密对接。本章通过对可行性进行多维论证，对系统日常研学中涉及的智能对话、联网搜索、知识库文档自动解析分块及参数热更新配置等核心功能进行深入建模与分析，并利用规范的用例图和核心用例表进行严密表述。最后，分析了安全性及兼容性等非功能性要求，并总结本章工作，为后文的系统总体设计奠定科学的业务需求基石。")
...
replace_simple_p(doc.paragraphs[134], "（3）自托管 SearXNG 联网搜索：在问答流程中若用户要求或大模型判断需联网，后端自动抓取自建自托管 SearXNG 引擎 of 搜索结果，格式化为标题、摘要、网页链接的引用卡片，拼接进上下文，实现信源精准溯源。")
...
# --- CORRECTED INDEX ALIGNMENTS FOR DB SECTION (P[139] to P[148]) ---
# P[139]: Module 4
replace_simple_p(doc.paragraphs[139], "4. 学习画像模块")

# P[140]: Module 4 text 1
replace_simple_p(doc.paragraphs[140], "（1）用户提问主题自动归类：学生每次提问都会记录在 user_question_log 中，后台服务通过调用内置 light-weight 分类器（TopicClassifier），基于 NLP 主题模型自动将其划分为具体的课程学科模块或知识点范畴。")

# P[141]: Module 4 text 2
replace_simple_p(doc.paragraphs[141], "（2）D3可视化与深度画像分析：系统根据主题分类汇总学生的提问频数与掌握度得分，生成层次 hierarchical JSON，前端在 profile.html 中使用 D3.js 动态绘制华丽的交互式树状图与堆积图；同时提供沉浸式画像日志分析与薄弱课件章节推荐，打通“诊断、检索、自学”的教务闭环。")

# P[142]: Module 5
replace_simple_p(doc.paragraphs[142], "5. 系统设置模块")

# P[143]: Module 5 text 1
replace_simple_p(doc.paragraphs[143], "（1）API 密钥与模型热更新：系统设计了无需重启服务的模型热更新引擎。管理员在 Web 端管理后台输入新的 AI API 接口地址、API 密钥与模型名称，提交后后台直接覆盖 Redis 中的 AiProperties 缓存，实现秒级热生效。")

# P[144]: Module 5 text 2
replace_simple_p(doc.paragraphs[144], "（2）System Prompt 在线编辑：管理员或教师可在线微调全局系统提示词（System Prompt），直接调控大语言模型的性格、答复格式和字数范围限制，更新后的 Prompt 实时应用到新建会话的上下文装配链条中。")

# P[145]: Heading 2 "3.3数据库设计"
replace_simple_p(doc.paragraphs[145], "3.3 数据库设计")

# P[146]: Normal "数据库设计确保系统..."
replace_simple_p(doc.paragraphs[146], "数据库设计是 Study AI 系统稳定运行与高效查询的底层技术基石。对于包含流式对话 and 大规模 RAG 切片检索的系统，需要合理设计关系表模式，保障会话与消息级联删除的高效性、切片查询的高并发性，并实现精确的主题分类日志索引。")
...
replace_simple_p(doc.paragraphs[148], "系统的概念设计（E-R图）主要描述了用户、对话会话、对话消息、教学文档、文档切片和提问日志等实体间的对应关系。用户实体具有与提问日志、文档和会话的一对多关联，会话与消息、文档与切片也呈现一对多的级联级关系。系统的总体概念 E-R 图如图 3-2 所示。")
...
# P[184]: Normal
replace_simple_p(doc.paragraphs[184], "自托管智能学业辅助平台的详细设计与实现章节主要介绍“智能问答 SSE 流式对话模块”与“RAG 知识库检索与降级匹配模块”的详细设计。每个核心模块都配备了规范的 UML 时序图、程序流程图 and 实现说明，以清晰表达核心多线程逻辑与容错算法的编码实践。")
...
def set_cell_text(cell, text):
    ...
    if not p.runs:
        ...
        for r in p.runs[1:]:
            r.text = ""
    # Completely remove any subsequent paragraphs in the cell to prevent leftover template content
...
print("Text, tables and TOC successfully written.")
...
steps = [
    ...
    ("4. 双路降级检索 RAG 拼接上下文 Prompt", 380, 380, 360),
...
print("All custom diagrams successfully drawn inside media.")
...
if os.path.exists(doc_xml_path):
    ...
    content = content.replace("甜品", "学业辅助")
    content = content.replace("商品分类", "文档分类")
    content = content.replace("商品", "文档")
...
print(f"COMPLETED! Final perfect DOCX written to {target_docx}!")
